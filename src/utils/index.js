 ...
 *  当 array 有子属性为数组时，进行深层过滤，对数组内对象进行过滤
 * @param refer 后台返回的菜单结构 json树
 * @param array 前台定义的菜单结构 json树
 * @returns {*}
 */
export function levelMatch(refer, array) {
    array = array.filter(a => {
        for(let i=0,l=refer.length; i<l;i++) {
            let r = refer[i];
            if(a.name === r.name) {
                if(a.children && r.children && a.children instanceof Array && r.children instanceof Array) {
                    a.children = levelMatch(r.children, a.children); // 过滤对象内数组
                    return true; // 对象匹配返回
                } else {
                    delete a.children; // 移除 children
                    return true; // 对象匹配返回
                }
            }
        }
